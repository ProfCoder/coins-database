...
function EditCoin() {
    ...
    const {id} = useParams(); // id should be at he beginning as it will be used below for coin selection 
    let currentCoin = coins.find(item => item.id === Number(id)) || {};

    const [name, setName] = useState(currentCoin.coin_name);
    const [category, setCategory] = useState(currentCoin.category);
    const [shortDescr, setShortDescr] = useState(currentCoin.short_desct);
    const [longDescr, setLongDescr] = useState(currentCoin.coin_desc);
    const [country, setCountry] = useState(currentCoin.country);
    const [composition, setComposition] = useState(currentCoin.composition);
    const [denomination, setDenomination] = useState(currentCoin.denomination);
    const [quality, setQuality] = useState(currentCoin.quality);
    const [year, setYear] = useState(currentCoin.coin_year);
    const [weight, setWeight] = useState(currentCoin.weight);
    const [price, setPrice] = useState(currentCoin.price);
    const [frontImage, setFrontImage] = useState(currentCoin.coin_front_image);
    const [reverseImage, setReverseImage] = useState(currentCoin.coin_reverse_image);
